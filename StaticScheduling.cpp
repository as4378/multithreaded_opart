...
int main(int argc, char** argv){
	...
	int factor = num_points / threads;
	int rem = num_points % threads;
	...
	for (int i=0;i<threads;i++) {
		struct params* arg = (struct params*)malloc(sizeof(struct params));
		arg->id = i;
		arg->start = factor * i;
		arg->end = arg->start + factor;
		arg->total = num_points;
		if (i == (threads - 1)){
			arg->end = num_points;
		}
    	if (pthread_create(&worker_thread[i], NULL, do_work, (void*)arg)){
